...
auto board_to_string(const Board& board) -> std::string {
    std::string result = "   ";
    for (char col : std::views::iota('A', 'I')) { result += std::format("{} ", col); }
    result += "\n";
    ...
    for (int const row_index : std::views::iota(0, 8)) {
        ...
        for (int const col_index : std::views::iota(0, 8)) {
            ...
            if ((row_index + col_index) % 2 == 0) {
                ...
                symbol = piece_symbol(board.is_black_piece(Position{col_index, row_index}),
                                      board.is_dame_piece(Position{col_index, row_index}));
            } else {
                symbol = " ";
            }
...
TEST_CASE("Explorer - Basic movement tests", "[Explorer]") {
    SECTION("White pion normal moves") {
        ...
        const Pieces pieces = {{focus, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}};
        ...
        for (std::size_t i = 0; i < options.size(); ++i) { positions.insert(options.get_position(i)); }
    ...
    SECTION("Black pion normal moves") {
        const Position focus{"C4"};
        const Pieces pieces = {{focus, PieceInfo{.color = PieceColor::BLACK, .type = PieceType::PION}}};
    ...
    SECTION("White dame normal moves") {
        const Position focus{"C4"};
        const Pieces pieces = {{focus, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::DAME}}};
...
TEST_CASE("Explorer - Pion capture tests", "[Explorer]") {
    ...
    SECTION("Multiple capture opportunities") {
        ...
        for (std::size_t i = 0; i < options.size(); ++i) {
            ...
            REQUIRE(captured_pieces.size() == 1);
            if (captured_pieces[0] == Position{"B3"} && target_position == Position{"A2"}) { found_b3_capture = true; }
            if (captured_pieces[0] == Position{"D3"} && target_position == Position{"E2"}) { found_d3_capture = true; }
        }
...
TEST_CASE("Explorer - Error handling tests", "[Explorer]") {
    SECTION("Empty position throws exception") {
        ...
        const Pieces pieces = {{Position{"C4"}, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}};
    ...
    SECTION("Mixed piece types on board") {
        ...
        const Pieces mixed_pieces = {{pion_focus, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}},
...
TEST_CASE("Explorer - Movement range comparison", "[Explorer]") {
    SECTION("Dame vs Pion movement range") {
        ...
        const Pieces pion_pieces = {{focus, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}};
        ...
        const Pieces dame_pieces = {{focus, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::DAME}}};
        ...
        for (std::size_t i = 0; i < pion_options.size(); ++i) { pion_positions.insert(pion_options.get_position(i)); }

        std::set<Position> dame_positions;
        for (std::size_t i = 0; i < dame_options.size(); ++i) { dame_positions.insert(dame_options.get_position(i)); }

        // All pion moves should also be valid dame moves
        for (const auto& pos : pion_positions) { REQUIRE(dame_positions.count(pos) == 1); }
    }
}
...
TEST_CASE("Explorer - Pion Edge case tests", "[PionAnalyzer]") {
    SECTION("Board boundaries") {
        ...
        const Pieces edge_pieces = {{edge_pos, PieceInfo{.color = PieceColor::BLACK, .type = PieceType::PION}}};
    ...
    SECTION("Corner position behavior") {
        ...
        const Pieces corner_pieces = {{corner_pos, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}};
    ...
    SECTION("Color dependent directions") {
        ...
        const Pieces white_pieces = {{white_pos, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}};
        ...
        for (std::size_t i = 0; i < white_options.size(); ++i) { white_moves.insert(white_options.get_position(i)); }
        ...
        const Pieces black_pieces = {{black_pos, PieceInfo{.color = PieceColor::BLACK, .type = PieceType::PION}}};
        ...
        for (std::size_t i = 0; i < black_options.size(); ++i) { black_moves.insert(black_options.get_position(i)); }
...
TEST_CASE("Explorer - Pion Advanced capture scenarios", "[PionAnalyzer]") {
    ...
    SECTION("Capture with no landing space") {
        ...
        const Pieces edge_pieces = {{edge_focus, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}},
                                    {Position{"B3"}, PieceInfo{.color = PieceColor::BLACK, .type = PieceType::PION}}};
        ...
        if (edge_options.has_captured()) { REQUIRE(!edge_options.empty()); }
    }
}
...
TEST_CASE("DameCaptureAnalyzer - Complex multi-capture scenario", "[DameCaptureAnalyzer]") {
    SECTION("Complex multi-capture test scenario") {
        ...
        for (const auto& [length, count] : counts) {
            INFO(std::format("  - {} sequences with {} captures each", count, length));
        }
        ...
        for (std::size_t i = 0; i < moves.size(); ++i) {
            ...
            INFO(std::format("  - Target: {} Captures: {} ({})", position.to_string(), captures_str,
                             captured_pieces.size()));
            std::cout << "  - Target: " << position.to_string() << " Captures: " << captures_str << " ("
                      << captured_pieces.size() << ")\n";
        }
        ...
        for (const auto& [expected_target, expected_captures] : expected_sequences) {
            ...
            if (!found) {
                ...
                FAIL(std::format("MISSING: Target {} with captures: {} ({})", expected_target, expected_caps_str,
                                 expected_captures.size()));
                all_verified = false;
            }
        }
        ...
        for (std::size_t i = 0; i < moves.size(); ++i) {
            ...
            if (!found_in_expected) {
                ...
                FAIL(std::format("UNEXPECTED: Target {} with captures: {} ({})", actual_position.to_string(),
                                 actual_caps_str, actual_captured_pieces.size()));
                all_verified = false;
            }
        }
...
TEST_CASE("DameAnalyzer - Edge Cases and Comprehensive Coverage", "[DameAnalyzer]") {
    ...
    SECTION("Dame at board boundaries") {
        ...
        std::vector<Position> const edge_positions = {Position("B1"), Position("H1"), Position("A8"), Position("G8")};
...
TEST_CASE("Explorer - Dame complex capture sequences (from main)", "[Explorer]") {
    SECTION("Complex capture pattern with 22 unique sequences") {
        ...
        for (std::size_t i = 0; i < sequence_count; ++i) {
            ...
            for (const auto& piece : captured_pieces) { REQUIRE(piece.is_valid()); }
        }
    }
}
...
TEST_CASE("Explorer - Dame normal moves (from main)", "[Explorer]") {
    SECTION("Dame with friendly piece blocking some moves") {
        ...
        std::set<Position> expected_positions = {Position{"C4"}, Position{"E4"}, Position{"F3"}, Position{"G2"},
                                                 Position{"H1"}, Position{"C6"}, Position{"B7"}, Position{"A8"},
                                                 Position{"E6"}, Position{"F7"}, Position{"G8"}};
    ...
    SECTION("Pion multiple capture sequences") {
        ...
        for (std::size_t i = 0; i < sequence_count; ++i) {
            ...
            if (captured_pieces.size() == kExpectedTripleCaptureSize) {
                ...
                if ((static_cast<unsigned int>(captures.contains(Position{"D7"})) != 0U) &&
                    (static_cast<unsigned int>(captures.contains(Position{"B5"})) != 0U) &&
                    (static_cast<unsigned int>(captures.contains(Position{"B3"})) != 0U)) {
                    found_d7_b5_b3 = true;
                }
                if ((static_cast<unsigned int>(captures.contains(Position{"D7"})) != 0U) &&
                    (static_cast<unsigned int>(captures.contains(Position{"D5"})) != 0U) &&
                    (static_cast<unsigned int>(captures.contains(Position{"D3"})) != 0U)) {
                    found_d7_d5_d3 = true;
                }
                if ((static_cast<unsigned int>(captures.contains(Position{"F7"})) != 0U) &&
                    (static_cast<unsigned int>(captures.contains(Position{"F5"})) != 0U) &&
                    (static_cast<unsigned int>(captures.contains(Position{"F3"})) != 0U)) {
                    found_f7_f5_f3 = true;
                }
            }
        }
...
TEST_CASE("Explorer - Capture attempt with landing outside board", "[Explorer]") {
    SECTION("White pion at C2 trying to capture black pion at B1 (NW direction)") {
        ...
        const Pieces pieces = {{white_pos, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}},
...
TEST_CASE("Legals - Error handling and edge cases", "[Legals]") {
    ...
    SECTION("Test out of range exceptions for capture sequences") {
        ...
        CaptureSequences sequences = {{Position{"A2"}, Position{"B3"}}, {Position{"C4"}, Position{"D5"}}};
