...
TEST_CASE("Game - Default Constructor", "[Game]") {
    ...
    SECTION("Initial player is white") {
...
TEST_CASE("Game - Constructor with Board", "[Game]") {
    ...
    SECTION("Board is set correctly") {
        REQUIRE(game.board().hash() == custom_board.hash());
    }
...
TEST_CASE("Game - Copy functionality", "[Game]") {
    ...
    SECTION("Board state matches") {
        REQUIRE(copied.board().hash() == original.board().hash());
    }
    
    SECTION("Player matches") {
        REQUIRE(copied.player() == original.player());
    }
    
    SECTION("Move sequence matches") {
        REQUIRE(copied.get_move_sequence() == original.get_move_sequence());
    }
    
    SECTION("Looping status matches") {
        REQUIRE(copied.is_looping() == original.is_looping());
    }
}
...
TEST_CASE("Game - Undo functionality", "[Game]") {
    ...
    SECTION("Undo multiple moves") {
        ...
        // Undo all moves
        for (std::size_t i = 0; i < moves_made; ++i) {
            game.undo_move();
        }
    ...
    SECTION("Undo updates looping detection correctly") {
        ...
        for (int i = 0; i < 2 && game.move_count() > 0; ++i) {
            game.select_move(0);
        }
    ...
    SECTION("Undo after capture move") {
        ...
        auto initial_pieces_count = initial_board.get_pieces(PieceColor::BLACK).size() + 
                                  initial_board.get_pieces(PieceColor::WHITE).size();
        ...
        if (promotion_game.move_count() > 0) {
            ...
            if (promotion_game.move_count() > 0) {
                ...
                for (const auto& [pos, piece_info] : white_pieces) {
                    if (pos.y() != 0 && piece_info.type == PieceType::DAME) {
...
TEST_CASE("Game - Looping detection", "[Game]") {
    ...
    SECTION("Move count is zero when looping") {
        Pieces pieces = {
            {Position{"D7"}, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::DAME}},
...
TEST_CASE("Game - Print functions don't crash", "[Game]") {
    ...
    SECTION("Print choices doesn't crash") {
        REQUIRE_NOTHROW(game.print_choices());
    }
}
...
TEST_CASE("Game - Promotion mechanics", "[Game]") {
    SECTION("Piece promotion verification") {
        ...
        const Pieces test_pieces = {
            {Position{0, 1}, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}, // Can potentially move to row 0
            {Position{2, 3}, PieceInfo{.color = PieceColor::BLACK, .type = PieceType::PION}}  // Black piece for valid game
        };
    ...
    SECTION("Black piece promotion test") {
        ...
        const Pieces black_promotion_pieces = {
            ...
            {Position{0, 1}, PieceInfo{.color = PieceColor::WHITE, .type = PieceType::PION}}  // White piece for valid game
        };
