...
public:
    ...
    [[nodiscard]] static constexpr bool is_valid(CoordType x, CoordType y) noexcept {
        // Allow only black squares (odd sum of coordinates)
        return x >= 0 && x < static_cast<CoordType>(board_size) && 
            y >= 0 && y < static_cast<CoordType>(board_size) && 
            (x + y) % 2 == 1;
    }
    ...
    template<Coordinate CoordType>
    [[nodiscard]] static constexpr Position from_coords(CoordType x, CoordType y) {
    ...
    [[nodiscard]] static constexpr auto all_valid_positions() noexcept {
        ...
        for (int y = 0; y < static_cast<int>(board_size); ++y) {
            for (int x = 0; x < static_cast<int>(board_size); ++x) {
                if (is_valid(x, y) && index < max_positions()) {
...
namespace std {
    ...
    struct hash<Position> {
        [[nodiscard]] constexpr std::size_t operator()(const Position& p) const noexcept {
            return p.hash();
        }
    };
    ...
    struct formatter<Position> {
        ...
        auto format(const Position& pos, format_context& ctx) const {
            return format_to(ctx.out(), "{}", pos.to_string());
        }
    };
}