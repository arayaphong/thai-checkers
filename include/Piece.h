...
enum class PieceType : uint8_t { PION = 0, DAME = 1 };
...
consteval auto piece_color_count() noexcept -> std::size_t { return 2; }

consteval auto piece_type_count() noexcept -> std::size_t { return 2; }
...
template <PieceEnumType T> constexpr auto to_underlying(T value) noexcept -> std::underlying_type_t<T> {
...
struct PieceInfo {
    ...
    [[nodiscard]] constexpr auto is_white() const noexcept -> bool { return color == PieceColor::WHITE; }

    [[nodiscard]] constexpr auto is_black() const noexcept -> bool { return color == PieceColor::BLACK; }

    [[nodiscard]] constexpr auto is_pion() const noexcept -> bool { return type == PieceType::PION; }

    [[nodiscard]] constexpr auto is_dame() const noexcept -> bool { return type == PieceType::DAME; }

    // String representation
    [[nodiscard]] constexpr auto color_string() const noexcept -> std::string_view { return to_string(color); }

    [[nodiscard]] constexpr auto type_string() const noexcept -> std::string_view { return to_string(type); }

    // Hash support for modern containers
    [[nodiscard]] constexpr auto hash() const noexcept -> std::size_t {
        return static_cast<std::size_t>(to_underlying(color)) << 8 | static_cast<std::size_t>(to_underlying(type));
    }
};

// C++20 concepts for PieceInfo validation
template <typename T>
concept ValidPieceInfo = std::same_as<T, PieceInfo> && requires(T piece) {
